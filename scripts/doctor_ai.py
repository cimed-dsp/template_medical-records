...
#################################################################
''' This module creates a neural net and trains it using gradient descent.
The inputs are:
    - list of visit codes (seqs) from process_mimic.py
    - number of unique codes in visit codes (seqs) from process_mimic.py
    - list of labels codes (label) from process_mimic.py
    - number of unique codes in label codes (label) from process_mimic.py
    - output file name

The outputs:
    - model file as a compressed .npz

    The input lists have a nested structure. The structure is as follows:
    [patientA[visit1[code1, code2...], visit2[code3, code4,...]...],
        patientB[visit1[code1,...],...], ...]
    The codes were taken from the EMR data and processed into this format
    by lookup to get ICD9 codes.  Note: no other fields or data are used for
    this model! We're only using ICD9 Diagnostic codes by default.

    The ICD9 codes were first mapped to integers to keep the overhead down.
    The label codes could be anything.  Here, we decided to collapse
...
def load_embedding(infile):
    Wemb = np.array(json.load(open(infile, 'r'))).astype(config.floatX)
    return Wemb

def init_params(options):
    params = OrderedDict()
    timeFileTrain = options['timeFileTrain']
    ...
    if len(timeFileTrain) > 0:
        prevDimSize += 1 #We need to consider an extra dimension for the duration information
...
def build_model(tparams, options, W_emb=None):
    ...
    if len(options['timeFileTrain']) > 0:
        useTime = True
    else:
        useTime = False
...
def adadelta(tparams, grads, x, y, mask, lengths, cost, options, t=None, t_label=None):
    zipped_grads = [theano.shared(p.get_value() * numpy_floatX(0.), name=f'{k}_grad', borrow=True) for k, p in tparams.items()]
    running_up2 = [theano.shared(p.get_value() * numpy_floatX(0.), name=f'{k}_rup2', borrow=True) for k, p in tparams.items()]
    running_grads2 = [theano.shared(p.get_value() * numpy_floatX(0.), name=f'{k}_rgrad2', borrow=True) for k, p in tparams.items()]
    ...
    elif len(options['timeFileTrain']) > 0:
...
def load_data(seqFileTrain, seqFileTest, seqFileValid, labelFileTrain,\
    labelFileTest, labelFileValid, timeFileTrain, timeFileTest, timeFileValid):
    train_set_x = json.load(open(seqFileTrain, 'r'))
    valid_set_x = json.load(open(seqFileValid, 'r'))
    test_set_x = json.load(open(seqFileTest, 'r'))
    train_set_y = json.load(open(labelFileTrain, 'r'))
    valid_set_y = json.load(open(labelFileValid, 'r'))
    test_set_y = json.load(open(labelFileTest, 'r'))
    train_set_t = None
    valid_set_t = None
    test_set_t = None

    if len(timeFileTrain) > 0:
        train_set_t = json.load(open(timeFileTrain, 'r'))
        valid_set_t = json.load(open(timeFileValid, 'r'))
        test_set_t = json.load(open(timeFileTest, 'r'))
...
def train_doctorAI(\
        seqFileTrain='seqFileTrain.json', seqFileTest='seqFileTest.json',\
        seqFileValid='seqFileValid.json', inputDimSize=20000, labelFileTrain='labelFileTrain.json',\
        labelFileTest='labelFileTest.json', labelFileValid='labelFileValid.json', numClass=500,\
        outFile='outFile.txt', timeFileTrain='timeFileTrain.json',\
        timeFileTest='timeFileTest.json', timeFileValid='timeFileValid.json',\
        predictTime=False, tradeoff=1.0, useLogTime=True, embFile='embFile.txt',\
        embSize=200, embFineTune=True, hiddenDimSize=[200, 200], batchSize=100,\
        max_epochs=10, L2_output=0.001, L2_time=0.001, dropout_rate=0.5,\
        logEps=1e-8, verbose=False):
    options = locals().copy()
    ...
    trainSet, validSet, testSet = load_data(\
        seqFileTrain, seqFileTest, seqFileValid,\
        labelFileTrain, labelFileTest, labelFileValid,\
    ...
    for epoch in range(max_epochs):
        ...
        for index in random.sample(list(range(n_batches)), n_batches):
            ...
            if (iteration % 10 == 0) and verbose:
                print(f'epoch:{epoch}, iteration:{iteration}/{n_batches}, cost:{cost}')
            iteration += 1

        print(f'epoch:{epoch}, mean_cost:{np.mean(costVector)}')
        use_noise.set_value(0.)
        validAuc = calculate_auc(test_model, validSet, options)
        print(f'Validation cross entropy:{validAuc} at epoch:{epoch}')
        if validAuc < bestValidCrossEntropy:
            ...
            print(f'Test cross entropy:{testCrossEntropy} at epoch:{epoch}')
            tempParams = unzip(tparams)
            np.savez_compressed(outFile + '.' + str(epoch), **tempParams)
    print(f'The best valid cross entropy:{bestValidCrossEntropy} at epoch:{bestValidEpoch}')
    print(f'The test cross entropy: {testCrossEntropy}')

def parse_arguments(parser):
    parser.add_argument(\
        'seq_file_train',
        type=str,
        metavar='<visit_file_train>',
        help='The path to the file containing visit information of patients, train set')
    parser.add_argument(\
        'seq_file_test',
        type=str,
        metavar='<visit_file_test>',
        help='The path to the file containing visit information of patients, test set')
    parser.add_argument(\
        'seq_file_valid',
        type=str,
        metavar='<visit_file_valid>',
        help='The path to the file containing visit information of patients, valid set')
    parser.add_argument(\
        'n_input_codes',
        type=int,
    ...
    parser.add_argument(
        'label_file_train',
        type=str,
        metavar='<label_file_train>',
        help='The path to the file containing label information of patients, train set')
    parser.add_argument(
        'label_file_test',
        type=str,
        metavar='<label_file_test>',
        help='The path to the file containing label information of patients, test set')
    parser.add_argument(
        'label_file_valid',
        type=str,
        metavar='<label_file_valid>',
        help='The path to the file containing label information of patients, valid set')
    parser.add_argument(\
        'n_output_codes',
        type=int,
        metavar='<n_output_codes>',
    ...
    parser.add_argument(\
        ...
        type=str,
        default='',
        help='The path to the file containing durations between visits of patients, train set. If you are not using duration information, do not use this option')
    parser.add_argument(\
        '--time_file_test',
        type=str,
        default='',
        help='The path to the file containing durations between visits of patients, test set. If you are not using duration information, do not use this option')
    parser.add_argument(\
        '--time_file_valid',
        type=str,
        default='',
        help='The path to the file containing durations between visits of patients, valid set. If you are not using duration information, do not use this option')
    parser.add_argument(\
        '--predict_time',
        type=int,
        default=0,
        choices=[0, 1],
        help='Use this option if you want the GRU to also predict the time duration until the next visit (0 for false, 1 for true) (default value: 0)')
    parser.add_argument(\
        '--tradeoff',
        type=float,
        default=1.0,
        help='Tradeoff variable for balancing the two loss functions: code prediction function and duration prediction function (default value: 1.0)')
    parser.add_argument(\
        '--use_log_time',
        type=int,
        default=1,
    ...
    parser.add_argument(\
        ...
        type=str,
        default='',
        help='The path to the file containing the representation vectors of medical codes. If you are not using medical code representations, do not use this option')
    parser.add_argument(\
        '--embed_size',
        type=int,
        default=200,
        help='The size of the visit embedding before passing it to the GRU layers. If you are not providing your own medical code vectors, you must specify this value (default value: 200)')
    parser.add_argument(\
        '--embed_finetune',
        type=int,
        default=1,
        choices=[0, 1],
        help='If you are using randomly initialized code representations, always use this option. If you are using an external medical code representations, and you want to fine-tune them as you train the GRU, use this option as well. (0 for false, 1 for true) (default value: 1)')
    parser.add_argument(\
        '--hidden_dim_size',
        type=str,
        default='[200,200]',
        help='The size of the hidden layers of the GRU. This is a string argument. For example, [500,400] means you are using a two-layer GRU where the lower layer uses a 500-dimensional hidden layer, and the upper layer uses a 400-dimensional hidden layer. (default value: [200,200])')
    parser.add_argument(\
        '--batch_size',
        type=int,
        default=100,
    ...
    parser.add_argument(\
        ...
        type=float,
        default=0.001,
        help='L2 regularization for the softmax function (default value: 0.001)')
    parser.add_argument(\
        '--L2_time',
        type=float,
        default=0.001,
        help='L2 regularization for the linear regression (default value: 0.001)')
    parser.add_argument(\
        '--dropout_rate',
        type=float,
        default=0.5,
        help='Dropout rate between GRU hidden layers, and between the final hidden layer and the softmax layer (default value: 0.5)')
    parser.add_argument(\
        '--log_eps',
        type=float,
        default=1e-8,
    ...
    parser.add_argument(\
        '--verbose',
        action='store_true',
        help='Print output after every 10 mini-batches (default false)')
...
def main():
    ...
    train_doctorAI(
        seqFileTrain=args.seq_file_train,
        seqFileTest=args.seq_file_test,
        seqFileValid=args.seq_file_valid,
        inputDimSize=args.n_input_codes,
        labelFileTrain=args.label_file_train,
        labelFileTest=args.label_file_test,
        labelFileValid=args.label_file_valid,
        numClass=args.n_output_codes,
        outFile=args.out_file,
        timeFileTrain=args.time_file_train,
        timeFileTest=args.time_file_test,
        timeFileValid=args.time_file_valid,
